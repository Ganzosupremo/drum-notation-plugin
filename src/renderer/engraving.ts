...
function glyphFor(event: DrumEvent): string {
    ...
    if (event.technique === "rimshot") return GLYPHS.noteheadSlashedBlack1;
...
function fitSvgVertically(svg: SVGSVGElement, fallbackTop: number, fallbackHeight: number): void {
    const boxes = Array.from(svg.children).flatMap(child => {
        const graphics = child as SVGGraphicsElement;
        if (typeof graphics.getBBox !== "function") return [];
        try {
            const box = graphics.getBBox();
            return Number.isFinite(box.y) && Number.isFinite(box.height) ? [box] : [];
        } catch {
            return [];
        }
    });
    ...
    const padding = 8;
    const top = Math.min(fallbackTop, ...boxes.map(box => box.y - padding));
    const bottom = Math.max(fallbackTop + fallbackHeight, ...boxes.map(box => box.y + box.height + padding));
    const width = svg.viewBox.baseVal.width;
    svg.setAttribute("viewBox", `0 ${Math.floor(top)} ${width} ${Math.ceil(bottom - top)}`);
    svg.setAttribute("height", Math.ceil(bottom - top).toString());
}

export function renderEngravingDocument(
    ...
    const draw = () => {
        ...
        systems.forEach(system => {
            ...
            fitSvgVertically(svg, vertical.top, svgHeight);
        });
    };
