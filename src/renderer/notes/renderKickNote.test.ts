...
type RenderKickNote = typeof import("./renderKickNote").renderKickNote;
let renderKickNote: RenderKickNote;
...
before(async () => {
    ...
    const mod = await import("./renderKickNote");
    renderKickNote = mod.renderKickNote;
    const smufl = await import("../smufl");
    GLYPHS = smufl.GLYPHS;
});

describe("renderKickNote — normal", () => {

    test("emits a <text> element", () => {
        const svg = makeSVG();
        renderKickNote(svg as any, 50, 100, "normal");
    ...
    test("uses provided x and y coordinates", () => {
        const svg = makeSVG();
        renderKickNote(svg as any, 60, 110, "normal");
        const glyph = svg.children.find(c => c.tagName === "text" && c.classList.has("drum-glyph"));
        assert.ok(glyph);
        assert.equal(glyph.getAttribute("x"), "60");
...
describe("renderKickNote — ghost", () => {

    test("ghost wraps notehead in a <g> with opacity attribute", () => {
        const svg = makeSVG();
        renderKickNote(svg as any, 50, 100, "ghost");
...
describe("renderKickNote — accent", () => {

    test("accent emits a second <text> element for the accent mark", () => {
        const svg = makeSVG();
        renderKickNote(svg as any, 50, 100, "accent");
        const texts = svg.children.filter(c => c.tagName === "text");
    ...
    test("accent has more children than normal", () => {
        ...
        renderKickNote(svgNormal as any, 50, 100, "normal");
        const svgAccent = makeSVG();
        renderKickNote(svgAccent as any, 50, 100, "accent");
